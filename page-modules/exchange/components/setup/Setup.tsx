...
import { FieldExtraInfo, SetupFormFields, SetupFormInitialValue } from 'page-modules/exchange/config/config'
import { UseMutationSetupPolicy } from 'page-modules/exchange/hooks/mutations'
import { useRmsExchangePolicyData } from 'page-modules/exchange/hooks/queries'
...
import { DateFieldType, FormData, SetupPolicy, SetupPolicyDetail, SetupPolicyPayload } from './type/setup'
...
export default function Setup() {
    ...
    const { isLoading, isError } = useRmsExchangePolicyData(
        `${setupPolicyApiUrl}?rms_type=EXCHANGE`,
        handleQuerySuccess,
    )
    ...
    const setFormFieldInitialValues = (policyDetails: SetupPolicyDetail) => {
        formFields.forEach((formField) => {
            if (columnTypeMapping[formField.fieldKey] === 'DATE' && dateFieldMapping[formField.fieldKey]) {
                ...
                let values: string[] = []
                policyDetails['exclusive_catalogs'].forEach((product: any) => {
    ...
    const createPayload = (formData: FormData) => {
        ...
        return { rmsType: 'EXCHANGE', details: payload }
    }
        ...
        mutation.mutate(payload, {
            onSuccess: (data) => {
                if (data && data.data && data.data.successful) {
                    ...
                    toast.success('Exchange policy updated successfully!')
                } else {
                    toast.error('Exchange policy not updated')
                }
                queryClient.invalidateQueries(['get-rms-exchange-policy'])
            },
            onError: (error) => {
                // Handle error cases
                console.error(error)
